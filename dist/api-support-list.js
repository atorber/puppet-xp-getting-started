...
async function onLogin(user) {
    ...
    log.info('room count:', roomList.length);
    const contactList = await bot.Contact.findAll();
    log.info('contact count:', contactList.length);
}
...
async function onMessage(message) {
    ...
    //   log.info('forward this message to wechaty room!')
    // }
    try {
    ...
    if (/^luyuchao$/i.test(message.text())) {
        ...
        if (!contactCard) {
            log.warn('Bot', 'not found contact card');
            return;
        }
...
const bot = WechatyBuilder.build(wechatyConfig);
bot.on('scan', onScan);
...
bot.start()
    ...
    .catch(log.error);
//# sourceMappingURL=api-support-list.js.map